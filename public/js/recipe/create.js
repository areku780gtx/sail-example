window.onload = function () {
    ...
    Sortable.create(steps, {
        ...
        handle: ".handle",
        onEnd: function (evt) {
            var items = steps.querySelectorAll(".step-id");
            items.forEach(function (item, index) {
                item.querySelector(".step-number").innerHTML =
                    "手順" + (index + 1);
                item.querySelector(
                    ".step-input"
                ).name = `steps-array[${index}]`;
            });
        },
    });
    ...
    steps.addEventListener("click", function (evt) {
        if (
            ...
            evt.target.closest(".step-id").remove();
            var items = steps.querySelectorAll(".step-id");
            items.forEach(function (item, index) {
                item.querySelector(".step-number").innerHTML =
                    "手順" + (index + 1);
    ...
    addStep.addEventListener("click", function () {
        var stepCount = steps.querySelectorAll(".step-id").length;
    ...
    <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
        ...
        <p class ="step-number w-16">手順 ${stepCount + 1}</p>
            <input type="text" name="steps-array[${stepCount}]" placeholder="手順を入力" class="step-input border border-gray-300 p-2 mb-4 w-full rounded">
    ...
    addIngredient.addEventListener("click", function () {
        ...
        ingredient.innerHTML = `
         ...
         <input type="text" name="ingredients[${ingredientCount}][name]" placeholder="材料名" class="ingredient-name border border-gray-300 p-2 ml-4 mb-4 w-full rounded">
        
        <p class ="mx-2">:</p>
        <input type="text" name="ingredients[${ingredientCount}][quantity]" placeholder="分量" class ="ingredient-quantity border boder-gray-300 p-2 mb-4 w-full rounded">
