       ...
       @method('PATCH')
        ...
        <div class="grid grid-cols-2 rounded border border-gray-500 my-4">
            <div class="col-span-1">
                <img id ="preview" class ="object-cover rounded-t-lg  w-full aspect-video" src="{{$recipe['image']}}" alt="recipe-image">
...
@foreach($categories as $c)
    ...
    <option value="{{$c ['id']}}"{{($recipe['categories_id']??null)==$c['id']?'selected':''}}>{{$c['name']}}</option>
...
<div class="flex justify-center">
    ...
    @foreach($recipe['steps'] as $i=>$os)
        ...
        <div class ="step-id flex justify-between items-center mb-2">
            ...
            <input type="text" name="steps-array[{{$i}}]" value="{{$os['description']}}" placeholder="手順を入力" class="step-input border border-gray-300 p-2 mb-4 w-full rounded">
